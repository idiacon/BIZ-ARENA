...
function renderGameMarketRail() {
  ...
  const contracts = (room.contractBoard || []).slice(0, 2);
  const step = isFactoryRoom() ? nextGuidedFactoryStep() : null;
  ...
  const teacherCopy = player.isHost
  ...
  elements.gameMarketRail.innerHTML = `
    ...
    <section class="side-rail-card">
      <h3>${player.isHost ? 'Преподаватель' : 'Помощь'}</h3>
      ${teacherCopy}
    </section>
    <section class="side-rail-card">

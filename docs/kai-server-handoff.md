...
На `/client` ученик может выбрать явно опубликованное преподавателем лобби во вкладке `Доступные занятия`. Публичный каталог не содержит кода комнаты, имён учеников или данных учётной записи преподавателя; вход подтверждается отдельным пятисимвольным кодом. QR и ручной ввод кода продолжают работать.
